...
class ECKey():
    ...
    def create_adaptor_signature(self, T, msg, aux=None):
        """Create a Schnorr signature (see BIP340)."""
        assert self.valid
        assert len(msg) == 32
        import secrets
        while True:
            ...
            R = SECP256K1.affine(SECP256K1.mul([(SECP256K1_G, kp)]))
            k = kp if SECP256K1.has_even_y(R) else SECP256K1_ORDER - kp
            R = R if SECP256K1.has_even_y(R) else SECP256K1.negate(R)
            
            R_T = SECP256K1.affine(SECP256K1.add(R, T))
            if SECP256K1.has_even_y(R_T):
                break
            aux = None
            
        e = int.from_bytes(TaggedHash("BIP0340/challenge", R_T[0].to_bytes(32, 'big') + self.get_pubkey().get_bytes() + msg), 'big') % SECP256K1_ORDER
        # e = int.from_bytes(TaggedHash("BIP0340/challenge", R[0].to_bytes(32, 'big') + T[0].to_bytes(32, 'big') + self.get_pubkey().get_bytes() + msg), 'big')         % SECP256K1_ORDER
        return R, ((k + (e * self.secret)) % SECP256K1_ORDER).to_bytes(32, 'big')
